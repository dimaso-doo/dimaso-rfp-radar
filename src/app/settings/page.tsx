...
export default function Settings() {
  ...
  return <>
    ...
    <div className="max-w-2xl space-y-4">
      <section className="card p-5"><h2 className="font-bold">Tavily Search API</h2><p className="mb-4 mt-1 text-sm text-[#748079]">Primary provider for finding current public RFP candidates across the web.</p>{badge(tavilyConnected)}</section>
      <section className="card p-5"><h2 className="font-bold">Brave Search fallback</h2><p className="mb-4 mt-1 text-sm text-[#748079]">Used only if Tavily is not configured.</p>{badge(braveConnected)}</section>
      ...
      <section className="card p-5"><h2 className="font-bold">Daily email digest</h2><p className="mb-4 mt-1 text-sm text-[#748079]">Sends good fits and new review candidates after the broad scan and target-organization scan.</p>{badge(digestConnected)}<div className="mt-4 space-y-2 rounded-lg bg-[#f4f6f3] p-3 text-sm"><div className="flex items-center justify-between"><span>Broad web scan</span><b>08:00 Europe/Belgrade</b></div><div className="flex items-center justify-between"><span>Target org scan</span><b>08:10 Europe/Belgrade</b></div><div className="flex items-center justify-between"><span>Email digest</span><b>08:45 Europe/Belgrade</b></div></div></section>
    </div>
  </>;
}
